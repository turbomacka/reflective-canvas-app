...
export default function Admin() {
  ...
  const loadLogs = async () => {
    ...
    const { data } = await supa.from('conversation_logs')
      .select('first,second,feedback,created_at,delta_seconds')
      .eq('slug', slug)
  ...
  return (
    <main className="p-8 max-w-5xl mx-auto space-y-10 font-sans">
        ...
        <div className="flex flex-wrap gap-3">
          {pages.map(p=>(
            <div key={p.slug} className="flex items-center gap-2">
              <Button onClick={()=>edit(p.slug)}>{p.slug}</Button>
      ...
      <Card className="shadow-lg rounded-2xl border border-gray-100 p-6 space-y-4">
        <input className="w-full border p-3 rounded-lg" placeholder="slug" value={slug} onChange={e=>setSlug(e.target.value)}/>
        <input className="w-full border p-3 rounded-lg" placeholder="titel" value={title} onChange={e=>setTitle(e.target.value)}/>
        ...
        <textarea className="w-full border p-3 rounded-lg h-32" placeholder="Fråga" value={question} onChange={e=>setQuestion(e.target.value)}/>
      ...
      {pageUrl && (
        <Card className="shadow-lg rounded-2xl border border-gray-100 p-6 space-y-6 relative">
          <CardHeader><CardTitle className="text-2xl">Inställningar & Loggar</CardTitle></CardHeader>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="border-b">
              ...
              <TabsContent value="logs">
                <div className="flex gap-3 my-4">
                  <Button onClick={loadLogs}><Eye size={16}/> Visa</Button>
                  <Button onClick={downloadLogs} disabled={logs.length===0}><Download size={16}/> Ladda ner</Button>
                  <Button variant="destructive" onClick={handleDeleteLogs} disabled={logs.length===0}><Trash2 size={16}/> Radera</Button>
                </div>
                {logsLoading && (
                  ...
                  <Card key={i} className="mb-4">
                    <CardContent>
                      <p className="text-sm text-gray-500">
                        {new Date(l.created_at).toLocaleString()} — Tid: {l.delta_seconds ?? '?'} sek
                      </p>
              ...
              <TabsContent value="insights">
                <div className="flex gap-3 my-4">
                  ...
                  >
                    <Sparkle size={16}/> Hämta insikter
                  </motion.button>
                </div>
                {summaryLoading && (
                  <div className="absolute inset-0 bg-white bg-opacity-80 flex flex-col items-center justify-center rounded-2xl">
